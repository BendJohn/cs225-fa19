...
class KDTree
{
  ...
  private:
     ...
     */    
    /** Recursively creates root node */
    KDTreeNode* createTree(vector<Point<Dim>>&, int start, int end, int dim); 

    /** Recursively creates root node
     *  Adapted from GforG */
    Point<Dim> kthSmallest(vector<Point<Dim>>&, int start, int end, int dim, int k);

    /** Lomuto Partition (GforG) */
    int partition(vector<Point<Dim>>&, int start, int end, int dim);

    /** Find Nearest Neighbor Helper */
    Point<Dim> findNearestNeighbor(KDTreeNode* subroot, Point<Dim> query, int dim) const;

    /** To copy tree */
    KDTreeNode* treeCreate(const KDTreeNode& oth) const;
