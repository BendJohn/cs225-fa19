...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
	...
	return subroot->elements[first_larger_idx].value;
     ...
     // If it is not in the node, go to its child
     if (!subroot->is_leaf) {
	return find(subroot->children[first_larger_idx], key);
     // If we are at a leaf node, it isn't in the tree
     } else {
	return V();
     }
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    // If the key does exist, change the value.
    if (first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx].key == pair.key) {
	subroot->elements[first_larger_idx].value = pair.value;
	return;
    }
    // If subroot is a leaf node and the key doesn't exist, add it
    if (subroot->is_leaf) {
