...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1_upper.begin(), word1_upper.end(), word1_upper.begin(), [] ( char c ) { return ( std::toupper( c ) ); });
    std::transform(word2_upper.begin(), word2_upper.end(), word2_upper.begin(), [](unsigned char c) -> unsigned char { return std::toupper(c); });

    // Find the pronounciations mapped to the word
    auto it1 = dict.find(word1_upper);
    auto it2 = dict.find(word2_upper);

    if (it1 == dict.end() || it2 == dict.end() || it1->second.size() != it2->second.size()) {
	return false;
    }

    // If pronunciation is different length its false
