...
class BinaryTree
{
	...
	bool isOrderedRecursive(Node* subRoot, int min, int max) const;
	void getPaths(Node* subRoot, std::vector<T> curr_path, std::vector<std::vector<T>>& paths) const;
	int sumDistances(Node* subRoot, int dist, int & sumDist) const;
