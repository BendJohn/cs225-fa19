                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (!mergeQueue.empty() && (singleQueue.empty() || mergeQueue.front()->freq < singleQueue.front()->freq)) {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	TreeNode* merged = new TreeNode(smallest->freq.getFrequency() + smallest2->freq.getFrequency());
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
	curr_bit = bfile.getNextBit();
	if (curr_bit) { current = current->right; }
